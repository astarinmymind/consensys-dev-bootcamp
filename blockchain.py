...
class Blockchain(object):
    ...
    #checks if chain is valid by looping through each block in chain and verifying hash and proof
    ...
    #loops through each neighboring node, downloads it's chain, verifies using valid_chain()
    #if valid chain is found AND its length is greater than own, replace own chain
    def resolve_conflicts(self):
        """
        Consensus Algorithm: resolves conflicts by replacing chain with longest one in network
        ...
        #only looking for chain longer than ours
        ...
        #replace chain if discovered new valid chain longer than own
        if new_chain:
...
#Generate globally unique address for this node (random)
node_identifier = str(uuid4()).replace('-', '')

#Instantiate Blockchain class
blockchain = Blockchain()

#Create /mine endpoint, GET request
...
#Create /transactions/new endpoint, POST request
...
#Create /chain endpoint, returns full blockchain
